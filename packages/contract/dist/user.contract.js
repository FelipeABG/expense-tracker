"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.userContract = void 0;
var core_1 = require("@ts-rest/core");
var zod_1 = require("zod");
var user_schema_1 = require("./schemas/user.schema");
var c = (0, core_1.initContract)();
exports.userContract = c.router({
    create: {
        method: "POST",
        path: "",
        summary: "Create a new user.",
        description: "Create a new user in the system, returning the created user. If the user already exists, returns 400 http code.",
        body: user_schema_1.UserSchema.omit({ id: true, role: true }),
        responses: {
            201: zod_1.z
                .object({
                message: zod_1.z.string(),
                user: user_schema_1.UserSchema.omit({
                    role: true,
                    password: true,
                }),
            })
                .describe("User successfully created."),
            400: zod_1.z
                .object({ message: zod_1.z.string().array() })
                .describe("Invalid email and/or password."),
            401: zod_1.z
                .object({ message: zod_1.z.string() })
                .describe("You must be authenticated to access this endpoint."),
            403: zod_1.z
                .object({ message: zod_1.z.string() })
                .describe("You do not have the required role to access this endpoint."),
        },
    },
    getAll: {
        method: "GET",
        path: "",
        summary: "Retrieve all users.",
        description: "Returns a list with all the users in the system. If there are no users, returns an empty list. **OBS**: You can optionally pass `limit` and `offset` as query parameters to paginate the response.",
        query: zod_1.z
            .object({
            limit: zod_1.z.coerce.number().default(100),
            offset: zod_1.z.coerce.number().default(0),
        })
            .optional()
            .describe("Pagination query parameters."),
        responses: {
            200: zod_1.z
                .object({
                message: zod_1.z.string(),
                users: user_schema_1.UserSchema.omit({ password: true }).array(),
            })
                .describe("Successfully returned the user list."),
            401: zod_1.z
                .object({ message: zod_1.z.string() })
                .describe("You must be authenticated to access this endpoint."),
            403: zod_1.z
                .object({ message: zod_1.z.string() })
                .describe("You do not have the required role to access this endpoint."),
        },
    },
    deleteById: {
        method: "DELETE",
        path: "/:id",
        summary: "Delete an user by it's id.",
        description: "Delete an user by it's id. If there is no such user, returns 404 http code.",
        pathParams: zod_1.z.object({
            id: zod_1.z.number(),
        }),
        responses: {},
    },
    deleteByEmail: {
        method: "DELETE",
        path: "/by-email/:email",
        summary: "Delete an user by it's email.",
        description: "Delete an user by it's email. if there is no such user, returns 404 http code.",
        pathParams: zod_1.z.object({
            email: zod_1.z.string().email(),
        }),
        responses: {},
    },
}, { pathPrefix: "/users" });
